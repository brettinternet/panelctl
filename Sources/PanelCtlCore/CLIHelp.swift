public enum CLIHelp {
    ...
    public static let version = "panelctl 0.3.15"
    ...
    public static func text(for command: String? = nil) -> String {
        ...
        case "blackout":
            ...
            display UUID. --keep-blackout-on-input keeps a partial blocking blackout
            installed during activity and restarts its Restore or Sleep timer; it
            cannot be combined with --dim-to. Working dimming always applies the same
            timer extension to partial selections, including hardware brightness.
            Full coverage always retains its fixed finite endpoint. Restore working
            dimming from the status menu or `panelctl app restore`.
            --caffeinate prevents idle system sleep (advanced). --keep-displays-awake
            keeps displays awake until --sleep-after; macOS may sleep the Mac sooner,
            and the assertion applies globally. By default, automatic treatment defers
            while another app keeps the display awake; --ignore-playback disables that
